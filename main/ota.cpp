...
bool Ota::CheckVersion() {
    ...
    req.url = check_version_url_;
    req.body = post_data_;
    auto response = http->sendRequest(req);
    if (response.error.length() > 0) {
        ESP_LOGE(TAG, "Failed to open HTTP connection: %s", response.error.c_str());
        delete http;
        return false;
    }
    delete http;
...
void Ota::Upgrade(const std::string& firmware_url) {
    ...
    auto http = new HttpClient();
    auto req = HttpRequest(HttpMethod::GET, firmware_url);

    if (http->openRequest(req) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open HTTP connection");
        delete http;
        return;
    }

    size_t content_length = http->getOpenContentLen();
    if (content_length == 0) {
    ...
    while (true) {
        ...
        if (!image_header_checked) {
            ...
            if (image_header.size() >= sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t)) {
                ...
                auto err = esp_ota_begin(update_partition, OTA_WITH_SEQUENTIAL_WRITES, &update_handle);
                if (err) {
                    esp_ota_abort(update_handle);
                    delete http;
                    ESP_LOGE(TAG, "Failed to begin OTA error: %d", err);
                    return;
                }
